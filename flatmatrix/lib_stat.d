...
void mean_cov_crosscorr_inplace_dim( bool unbiased = true, bool diag_only = false, bool transposed_data = false, bool do_parallel = false, T )
  ( in ref MatrixT!T m
    , ref MatrixT!T m_mean
    , ref MatrixT!T m_cov
...
// https://en.wikipedia.org/wiki/Covariance_and_correlation
{
  ...
  mean_cov_crosscorr_inplace_nogc!(unbiased, diag_only, transposed_data, do_parallel, T)( m, m_mean, m_cov, m_crosscorr );
}


void mean_cov_crosscorr_inplace_nogc( bool unbiased = true, bool diag_only = false, bool transposed_data = false, bool do_parallel = false, T )
  ( in ref MatrixT!T m
    , ref MatrixT!T m_mean
    , ref MatrixT!T m_cov
    , ref MatrixT!T m_crosscorr
  ...
  debug
    {
      ...
      assert( m_crosscorr.dim == m_cov.dim );
    }

  mean_cov_inplace_nogc!(unbiased, diag_only, transposed_data, do_parallel)( m, m_mean, m_cov );

  m_crosscorr.data[] = m_cov.data[];

  auto cov_data = m_cov.data.ptr;
  auto cc_data  = m_crosscorr.data.ptr;
  
  immutable one_d_C = q{
      immutable diag_ind = i*(nfeat+1);
      ...
      static if (!diag_only)
        {
          ...
          foreach (j; (i+1)..nfeat)
            cc_data[ j*nfeat + i ] = (cc_data[ i*nfeat + j ] /= (std_i * sqrt( cov_data[ j*(nfeat+1) ] ) ));
        }
  };

  if (do_parallel)
    {
      foreach (i; parallel( iota( nfeat ), /*workUnitSize:*/1 ))
        mixin( one_d_C );
    }
  else
    {
...
void mean_cov_inplace_dim( bool unbiased = true, bool diag_only = false, bool transposed_data = false, bool do_parallel = false, T )
  ( in ref MatrixT!T m
    , ref MatrixT!T m_mean
    , ref MatrixT!T m_cov
    )
  @safe
{
  ...
  mean_cov_inplace_nogc!( unbiased, diag_only, transposed_data, do_parallel, T )( m, m_mean, m_cov );
}

void mean_cov_inplace_nogc( bool unbiased = true, bool diag_only = false, bool transposed_data = false, bool do_parallel = false, T )
  ( in ref MatrixT!T m
    , ref MatrixT!T m_mean
    , ref MatrixT!T m_cov
    )
 @trusted
{
  ...
  auto m_data_0    = m.data;
  auto mean_data_0 = m_mean.data;
  auto cov_data_0  = m_cov.data;

  mean_data_0[] = cast( T )( 0.0 );
  cov_data_0[]  = cast( T )( 0.0 );

  auto m_data    = m.data.ptr;
  auto mean_data = m_mean.data.ptr;
  auto cov_data  = m_cov.data.ptr;
  ...
  static if (transposed_data)
    {
      static if (do_parallel)
        {{
            ...
            static if (!diag_only)
              {
                scope ab_app = appender!(size_t[2][]);
                foreach (a; 0..nfeat)
                  foreach (b; a+1..nfeat)
                  {
                    size_t[2] ab = [a,b];
                ...
                foreach (ab; parallel( ab_app.data, /*workUnitSize:*/1 ))
                  {
                    ...
                    immutable am_1 = am_0 + nsample;

                    size_t bm = b * nsample;

                    auto acc_nondiag = cast(T)( 0.0 );
                    foreach (am; am_0..am_1)
                      {
                        acc_nondiag += m_data[ am ] * m_data[ bm++ ];
                      }

                    cov_data[ a * nfeat + b ] = acc_nondiag;
                  }
              }
          }}
      else
        {{
          ...
          debug assert( off_cov == cov_data_0.length );
          }}
    }
  else
    {
      static if (do_parallel)
        {{
            int i = 1;
            assert( --i, "parallel not implemented, consider transposed+parallel" );
          }}
      else
        {{
            for (size_t im = 0; im < n; )
...
(bool unbiased = true, bool diag_only = false, bool transposed_data = false, bool do_parallel = false, T)
  ( in ref MatrixT!T m
    , in size_t[] subset
    , ref MatrixT!T m_mean
    , ref MatrixT!T m_cov )
   @safe
{
  ...
  mean_cov_inplace_nogc!( unbiased, diag_only, transposed_data, do_parallel, T )( m, subset, m_mean, m_cov );
}
    ...
    , ref MatrixT!T m_cov ) @trusted
{
  ...
  static if (transposed_data)
                ...
                foreach (ab; parallel( ab_app.data, /*workUnitSize:*/1 ))
                  {
                    ...
                    foreach (iss; subset)
                      {
                        acc_nondiag += m_data[ am_0 + iss ] * m_data[ bm_0 + iss ];
                      }
  ...
  else
    {
      static if (do_parallel)
        {
          assert( false, "xxx not implemented. Consider transposed_data" );
        }
      else
        {
          foreach (iss; subset)
...
unittest // ------------------------------
{
  ...
  {
    if (verbose)
      writeln("---------- Test mean_cov (transposed data, do_parallel)");
    ...
    enum unbiased = true, diag_only = false
      , transposed_data = true // this one
      , do_parallel     = true // and this one
      ;

    scope auto m_transpose = m.transpose;
    
    mean_cov_inplace_dim!(unbiased, diag_only, transposed_data, do_parallel)( m_transpose, m_mean, m_cov );
    ...
    */
...

    {
      ...
      mean_cov_inplace_dim!(/*unbiased:*/true, /*diag_only:*/true, transposed_data, do_parallel)( m_transpose, m_mean2, m_cov2 );
...
*/
    ...
    {
    if (verbose)
      writeln("---------- Test mean_cov with subset (transposed data, do_parallel)");
    ...
    enum unbiased = true, diag_only = false, transposed_data = true, do_parallel = true;
    
    auto m_transpose = m.transpose;
    
    mean_cov_inplace_dim!(unbiased, diag_only, transposed_data, do_parallel)( m_transpose, subset, m_mean, m_cov );
...
*/
...

    {
      ...
      mean_cov_inplace_dim!(/*unbiased:*/true, /*diag_only:*/true, transposed_data, do_parallel)( m_transpose, subset, m_mean2, m_cov2 );
